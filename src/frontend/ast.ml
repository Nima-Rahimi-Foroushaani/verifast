...
(* This gross duplication is needed because OCaml recursive modules should be defined with an explicit signature. TODO: Fix it with ppx *)
module rec Ast_ : AST_ = struct
(** Types as they appear in source code, before validity checking and resolution. *)
type type_expr = (* ?type_expr *)
    StructTypeExpr of loc * string option * (string list (* type parameters *) * field list) option * struct_attr list * type_expr list (* type arguments *)
  | UnionTypeExpr of loc * string option * field list option
...
and TypingBounds : (Set.S with type elt = Ast_.expr * Ast_.type_expr) = Set.Make(struct
  type t = Ast_.expr * Ast_.type_expr
  let compare x y = 0
end)
